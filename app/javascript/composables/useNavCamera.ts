...
// Réglages caméra de la navigation (zoom, relief 3D), ajustables en séance via le
// panneau de commandes et reportés sur le profil. Partagé entre navigation libre et
...
// réglage d'inclinaison. Le relief est reporté sur le profil au relâchement
// (persistTerrain) ; le zoom ne l'est QUE manuellement via saveZoomToProfile, pour
// ne pas écraser le réglage par défaut par un zoom ponctuel de la séance.
...
export function useNavCamera(deps: {
  ...
  // Persiste le relief sur le profil. Le zoom n'est PAS capturé ici : on réécrit la
  // valeur déjà enregistrée (navPrefs.zoom) pour qu'un réglage de relief n'embarque
  // pas le zoom courant de la séance.
  function persistTerrain() {
    persistNavCamera(navPrefs.zoom, terrain3d.value)
  }
  ...
  function saveZoomToProfile() {
    persistNavCamera(camZoom.value, terrain3d.value)
  ...
  return {
    ...
    onZoomInput, applyTerrain, toggleTerrain, persistTerrain, saveZoomToProfile,
  }
}
