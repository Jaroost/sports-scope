...
export interface UserPreferences {
  ...
  navigation: { default_style: MapStyleId; zoom: number; pitch: number; terrain: boolean; nav_fps: number; line_width: number; line_color: string; line_opacity: number; turn_alert_m: number; turn_hint_m: number; turn_urgent_m: number; turn_repeat_ms: number; turn_repeat_urgent_ms: number; turn_green_hold_m: number; turn_green_hold_s: number; sound_volume: number; turn_marker_size: number; turn_marker_color: string; turn_marker_icon_color: string; radar_always_visible: boolean; radar_close_m: number }
  display: {
    default_sport: Sport
