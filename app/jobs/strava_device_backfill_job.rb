# Récupère en masse le matériel d'enregistrement (`device_name`) des activités
# Strava d'un utilisateur, en respectant le rate limit Strava (~1 requête par
# activité, endpoint `/activities/:id`). Traite un lot puis se ré-enfile : le run
# reste résumable même sans queue durable, puisqu'on ne cible que les activités
# `device_unchecked`. Calqué sur StravaStreamsBackfillJob.
class StravaDeviceBackfillJob < ApplicationJob
  queue_as :default
  ...
  def perform(run_id)
    ...
    user = run.user
    fetcher = StravaActivityDetailFetcher.new(user)
    run.update!(status: "running", rate_limited_until: nil)

    pending = user.strava_activities.device_unchecked.order(started_at: :desc)
    return run.update!(status: "completed") if pending.none?

    pending.limit(BATCH).each do |activity|
      begin
        fetcher.fetch_and_store_device!(activity)
      rescue StravaActivityDetailFetcher::ApiError => e
        return reschedule(run, e) if e.rate_limited?

        # 404 / autre : détail illisible. On marque l'activité vérifiée ("") pour ne
        # pas boucler dessus, et on note l'erreur.
        activity.update_column(:device_name, "")
    ...
    if user.strava_activities.device_unchecked.exists?
