# Récupère en masse les vignettes des photos des activités Strava d'un utilisateur
# (~1 requête par activité qui annonce des photos), en respectant le rate limit.
# Traite un lot puis se ré-enfile : le run reste résumable même sans queue durable,
# puisqu'on ne cible que les activités `photos_pending`. Calqué sur
# StravaStreamsBackfillJob.
class StravaPhotosBackfillJob < ApplicationJob
  queue_as :default
  ...
  def perform(run_id)
    ...
    user = run.user
    fetcher = StravaPhotosFetcher.new(user)
    run.update!(status: "running", rate_limited_until: nil)

    pending = user.strava_activities.photos_pending.order(started_at: :desc)
    return run.update!(status: "completed") if pending.none?

    pending.limit(BATCH).each do |activity|
      begin
        fetcher.fetch_and_store!(activity)
      rescue StravaPhotosFetcher::ApiError => e
        return reschedule(run, e) if e.rate_limited?

        # 404 / autre : photos illisibles (activité privée, supprimée…). On marque
        # l'activité vérifiée pour ne pas boucler dessus, et on note l'erreur.
        activity.update_column(:photo_thumbs, [])
    ...
    if user.strava_activities.photos_pending.exists?
